...
function renderChatsList(filter = '') {
  ...
  filtered.forEach(([chatId, data]) => {
    ...
    // 🚀 السحر هون: تمرير الاسم والصورة محلياً لتجاوز طلب الفايربيس كلياً وتسريع الفتح 100 ضعف
    div.addEventListener('click', () => {
      openChat(chatId, data.friendUid, { name: data.friendName, photo: data.friendPhoto });
    });
...
let currentMessagesQuery = null; // متغير جديد لحفظ الاستعلام وإغلاقه بدقة لمنع التعليق

function detachMessages() {
  if (messagesRef) {
    // السطر السحري: إغلاق قسري وشامل لكل الاتصالات والاستعلامات المعلقة فور خروجك
    messagesRef.off(); 
...
async function openChat(chatId, friendUid, friendProfile = null) {
  // 🚀 1. الانتقال الفوري للواجهة بمجرد اللمس (صفر تأخير)
  renderScreenUI('chat');
  detachMessages();

  // 🚀 2. جلب البيانات بالخلفية إذا لم تكن متوفرة، دون إيقاف التطبيق
  if (!friendProfile) {
